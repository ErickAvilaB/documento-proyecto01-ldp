...
Esta sección especifica la \emph{sintaxis léxica} del lenguaje: alfabeto
permitido, reglas de escaneo, manejo de espacios y comentarios, y definición
formal de clases de tokens mediante expresiones regulares. La especificación
evita cualquier carácter fuera de ASCII básico para garantizar portabilidad con
\texttt{pdfLaTeX}.
...
\paragraph{Patrones (informativos).}
\begin{code}
	LINE_COMMENT   ::= ';' [^\n]*
	BLOCK_COMMENT  ::= '#|' (.| \n)*? '|#'       // no anidable
	WS              ::= [ \t\r\n]+               // ignorar
...
\noindent
\textit{Patrón ilustrativo (no vinculante):}
\begin{code}
	RESERVED ::=
	'add1' | 'sub1' | 'sqrt' | 'expt' |
	'head' | 'tail' | 'fst'  | 'snd'  |
...
\noindent
\textit{Patrones recomendados (orden importa):}
\begin{code}
	OP2 ::= '<=' | '>=' | '!='
	OP1 ::= [+\-*/=<>]
\end{code}
...
\subsection*{Convenciones semánticas (resumen)}
\begin{itemize}
